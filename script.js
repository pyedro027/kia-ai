...
function freteGratis() {
  return state.cart.reduce((s, i) => s + i.qty, 0) >= 2;
}
...
function updateFab() {
  ...
  if (state.cart.length > 0) {
    ...
    // frete gratis tag
    const existing = document.getElementById('freteTag');
    if (freteGratis()) {
      if (!existing) {
        const tag = document.createElement('span');
        tag.id = 'freteTag';
        tag.className = 'frete-gratis-tag';
        tag.textContent = '🎉 FRETE GRÁTIS';
        document.getElementById('fabCount').after(tag);
      }
    } else {
...
function openCheckout() {
  ...
  const savedEnd = localStorage.getItem('kiacai_endereco') || '';
  const savedNome = localStorage.getItem('kiacai_nome') || '';
  document.getElementById('endInput').value = savedEnd;
  document.getElementById('nomeInput').value = savedNome;

  document.getElementById('endInput').classList.remove('input-error');
  document.querySelector('.pgto-options').classList.remove('input-error');
...
function gerarPix() {
  ...
  const nome   = CONFIG.pix_nome;
  const cidade = CONFIG.pix_cidade;
...
function renderCheckoutResumo() {
  ...
  // Mostrar parabéns frete grátis
  const parabens = document.getElementById('freteParabens');
  if (freteGratis()) {
    parabens.classList.add('show');
  } else {
    parabens.classList.remove('show');
  }
}
function finalizarPedido() {
  const nomeInput = document.getElementById('nomeInput');
  const endInput = document.getElementById('endInput');
  const pgtoContainer = document.querySelector('.pgto-options');

  const nome = nomeInput.value.trim();
  const end  = endInput.value.trim();

  endInput.classList.remove('input-error');
  if (pgtoContainer) pgtoContainer.classList.remove('input-error');
  ...
  if (!state.pgto) { 
    if (pgtoContainer) pgtoContainer.classList.add('input-error'); 
    hasError = true;
  }
  if (hasError) return;

  localStorage.setItem('kiacai_endereco', end);
  if (nome) localStorage.setItem('kiacai_nome', nome);
  ...
  if (freteGratis()) {
    msg += `🎉 *FRETE: GRÁTIS (pedido com 2 ou mais itens!)*\n\n`;
  } else {
